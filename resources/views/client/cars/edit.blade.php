                ...
                <div class="max-w-xl mx-auto">
                    <form action="{{ route('cars.update', $car) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-6">
                            ...
                            <x-text-input id="model" class="block mt-1 w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm" type="text" name="model" :value="old('model', $car->model ?? '')" required autocomplete="model"/>
                        ...
                        <div class="mb-6">
                            ...
                            <div class="flex items-center space-x-4">
                                <select id="last_service_date" name="last_service_date" class="block mt-1 w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm" {{ old('no_service_checkbox', !$car->last_service_date ?? false) ? 'disabled' : '' }}>
                                    ...
                                    @for ($i = ($car->year ?? date('Y')) + 1; $i <= date('Y'); $i++)
                                        <option value="{{ $i }}" {{ old('last_service_date', $car->last_service_date ?? '') == $i ? 'selected' : '' }}>{{ $i }}</option>
                                    @endfor
                                </select>
